...
if (-not (Test-Path $ApkFinal)) {
    Write-Host ""
    Write-Host "  FAIL: Build failed - '$ApkFinal' not found after build_android.ps1." -ForegroundColor Red
...
Write-Host "  OK: APK built and signed in ${BuildSecs}s - ${ApkSizeKB}KB" -ForegroundColor Green
...
if ($LASTEXITCODE -ne 0) {
    Write-Host ""
    Write-Host "  FAIL: adb install failed (exit $LASTEXITCODE)." -ForegroundColor Red
...
$PidDisplay = if ($AppPid) { $AppPid } else { "resolving..." }
Write-Host "  OK: App launched (PID: $PidDisplay)" -ForegroundColor Green
Write-Host ""
...
if ($AppPid) {
    ...
    Write-Host "  (PID not resolved - streaming all errors as fallback)" -ForegroundColor DarkGray
