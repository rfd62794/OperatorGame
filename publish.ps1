<#
.SYNOPSIS
OperatorGame — Publish to itch.io via Butler.

.DESCRIPTION
Pushes the dist/ directory to itch.io using the Butler CLI.
...
.EXAMPLE
  .\publish.ps1              # Push dist/ to itch.io
  .\publish.ps1 -DryRun      # Print Butler command without uploading
  .\publish.ps1 -Build       # Run trunk build first, then publish
...
Write-Host "  OperatorGame - Publish to itch.io" -ForegroundColor Cyan
Write-Host "============================================================" -ForegroundColor Cyan
if ($DryRun) {
...
# ---------------------------------------------------------------------------
if ($Build) {
    Write-Host "[pre] Running trunk build..." -ForegroundColor Cyan
    Push-Location $RepoRoot
    & trunk build
    Pop-Location
    if ($LASTEXITCODE -ne 0) {
        Write-Host "  ERROR: trunk build failed. Aborting publish." -ForegroundColor Red
        exit $LASTEXITCODE
    }
    Write-Host ""
}
...
Write-Host "  Source : $DistDir" -ForegroundColor DarkGray
...
if ($DryRun) {
    Write-Host "  Would execute: $butlerExe push `"$DistDir`" $itchioTarget" -ForegroundColor Yellow
    Write-Host ""
...
& $butlerExe push $DistDir $itchioTarget
if ($LASTEXITCODE -ne 0) {
