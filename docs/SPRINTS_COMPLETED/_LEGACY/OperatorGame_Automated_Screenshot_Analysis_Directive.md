# Phase F.3: Automated Screenshot Analysis & UI Feedback Generation

**Status:** Post-Capture Analysis  
**Target:** Coding Agent analyzes 7 screenshots automatically, generates structured UI feedback  
**Scope:** No manual annotation — fully automated vision analysis  
**Output:** `UI_FEEDBACK_REPORT.md` with actionable egui fixes

---

## Part A: Agent Views Screenshots & Generates Observations

**Coding Agent Action:**

For each of the 7 PNG files in the latest `screenshots_uitree_*` folder, use Claude's vision API to:

1. **Analyze visual structure:**
   - What UI elements are visible (buttons, cards, text, progress bars)
   - Where are they positioned (top/middle/bottom, left/right alignment)
   - What is the visual hierarchy (sizes, colors, spacing)

2. **Identify layout issues:**
   - Elements that appear misaligned
   - Spacing/padding problems (too much/too little gap)
   - Text that's hard to read (font size, contrast)
   - Touch targets that seem too small
   - Overlapping or clipped content

3. **Note visual states:**
   - Active vs inactive elements (color changes, highlighting)
   - How tabs/buttons are visually distinguished
   - Consistency across tabs

4. **Generate egui constraint recommendations:**
   - Specific margin/padding values to fix issues
...
## Part B: Python Script for Batch Vision Analysis

**Create script:** `C:\Github\OperatorGame\scripts\auto_analyze_screenshots.py`
...
"""
    ...
    report += "- `*_analysis.json` files in this folder contain full structural breakdown per tab\n"
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        ...
        print("\nExample:")
        print("  python auto_analyze_screenshots.py screenshots_uitree_20260321_163927")
        sys.exit(1)
...
Write-Host "Analyzing screenshots in: $($latestFolder.Name)" -ForegroundColor Cyan

# Run automated vision analysis
python.exe .\scripts\auto_analyze_screenshots.py $latestFolder.FullName

# Verify output
Write-Host "`nAnalysis complete! Generated files:" -ForegroundColor Green
Get-ChildItem $latestFolder.FullName -Filter "*_analysis.json" | ForEach-Object { Write-Host "  ✅ $_" }
Get-ChildItem $latestFolder.FullName -Filter "UI_FEEDBACK_REPORT.md" | ForEach-Object { Write-Host "  ✅ $_" }
```
...
- ✅ **Individual analysis JSONs** — One `*_analysis.json` per screenshot with detailed observations
- ✅ **UI_FEEDBACK_REPORT.md** — Comprehensive report with cross-tab patterns, priority fixes, egui constraint recommendations
- ✅ **No manual annotation required** — Fully automated vision analysis

---

## Success Criteria

✅ Agent analyzes all 7 screenshots without manual input  
✅ Generates structured JSON feedback for each screenshot  
✅ Produces comprehensive markdown report with actionable fixes  
✅ Report includes egui constraint pseudo-code  
✅ Identifies cross-tab patterns and recurring issues  

---

**Execute Part C now to generate complete UI analysis!**
